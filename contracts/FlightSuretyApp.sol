...
contract FlightSuretyApp {
    ...
    uint256 public constant MULTIPARTY_CONSENSUS_COUNT = 4;
    uint256 public airlinesRegisteredCount = 1;
    ...
    event FlightRegistered(string flight);//Event triggered when flight is registered
    event PassengerInsured(); //Event triggered when passenger purchases insurance
    event InsureesCredited(string flight); //Event triggered when Passenger is paid insurance
    ...
    */
                            ...
                            requireIsOperational()
    {
        ...
        if(airlinesRegisteredCount < MULTIPARTY_CONSENSUS_COUNT) {
          success = flightSuretyData.registerAirline(airline, msg.sender);
        }else{
          ...
          for(uint c = 0; c < consensusVotes.length; c++) {
           ...
           require(!isDuplicate, "Caller has already called");

           consensusVotes.push(msg.sender);
           if (consensusVotes.length >= airlinesRegisteredCount.div(2)){
    ...
    */
    function fund ()
                  ...
                  requireIsOperational()
    {
        require(msg.value == REGISTRATION_FEE_AIRLINES, "Not enough Ether to fund airline.");
        require(flightSuretyData.isFunded(msg.sender) == false, "Airline is already funded");
        
        flightSuretyData.fund(msg.sender,REGISTRATION_FEE_AIRLINES);
    ...
    */
    ...
    {
        require(isFlightRegistered(airline, flight, timestamp) == false, "This flight is already registered");
        bytes32 key = getFlightKey(airline, flight, timestamp);
        flights[key] = Flight({
                                 isRegistered: true,
    ...
    */
    ...
    {
        require(msg.value <= MAX_INSURANCE_FEE, "Passengers can pay a max of 1 ETH");
        address(flightSuretyData).transfer(msg.value);
        flightSuretyData.buy(airline, flight, timestamp, passenger, msg.value);
...
contract FlightSuretyData {
    ...
    function isRegistered(address airline) public returns(bool);
    function isFunded(address airline) public returns(bool);
    function registerAirline (address airline, address caller) external returns(bool);
    function fund (address airline, uint256 fundAmount) public;
    function buy(address airline, string  flight, uint256 timestamp, address passenger, uint256 amount) public;
    ...
    function getPassengerCredits(address passenger) external view returns(uint);
}